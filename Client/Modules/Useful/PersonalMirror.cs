...
namespace Blaze.Modules
{
    class PersonalMirror : BModule
    {
        ...
        {
            Menu = new QMNestedButton(BlazeMenu.Renders, "Personal\nMirror", 4, 1, "Click to view options for personal mirror", "Personal Mirror");
            ...
            {
                MainConfig.Instance.CanPickupPersonalMirror = true;
                Refresh();
            }, delegate 
            {
                MainConfig.Instance.CanPickupPersonalMirror = false;
                Refresh();
            }, "Toggle being able to pickup your personal mirror", MainConfig.Instance.CanPickupPersonalMirror);

            new QMToggleButton(Menu, 1, 2, "Optimize Mirror", delegate
            {
                MainConfig.Instance.OptimizedPersonalMirror = true;
                Refresh();
            }, delegate
            {
                MainConfig.Instance.OptimizedPersonalMirror = false;
                Refresh();
            }, "Toggle optimizing your personal mirror", MainConfig.Instance.OptimizedPersonalMirror);
        }
        ...
        private void Refresh()
        {
            ...
            if (_mirror != null && PlayerUtils.CurrentUser() != null)
            {
                ...
                _mirror.GetOrAddComponent<VRCMirrorReflection>().m_ReflectLayers = Config.Main.OptimizedPersonalMirror ? BlazeInfo.optimizeMask : BlazeInfo.beautifyMask;
        ...
        private void ToggleMirror(bool state)
        {
            if (state)
            {
                ...
                pos.y += MainConfig.Instance.MirrorScaleY / 2;
                ...
                _mirror.transform.localScale = new Vector3(MainConfig.Instance.MirrorScaleX, MainConfig.Instance.MirrorScaleY, 1f);
