...
{
    class AntiCrash : BModule
    {
        private QMNestedButton Menu;

        public override void QuickMenuUI()
        {
            Menu = new QMNestedButton(BlazeMenu.Settings, "Anti\nCrash", 4, 0, "Blaze's Client Anti Crash Settings", "Anti Crash");
        ...
        internal static AntiCrashShaderPostProcess ProcessShaders(Renderer renderer, int currentNukedShaders, int currentShaderCount)
        {
            for (int j = 0; j < renderer.materials.Length; j++)
            {
                ...
                //Blacklist Check
                if (BlacklistedShaders.blockList.Contains(renderer.materials[j].shader.name.ToLower()))
                {
        ...
        internal static bool ProcessJoint(Joint joint)
        {
        ...
        /*internal static void ProcessAvatarWhitelist(VRC.Core.ApiAvatar avatar)
        {
            //Avatar doesn't exist in our whitelist - only ask if we want to add it
            if (Configuration.GetAntiCrashLimitsConfig().WhitelistedAvatars.ContainsKey(avatar.id) == false)
            {
                GeneralWrappers.AlertAction("Whitelist", $"Are you sure you want to add {avatar.name} to your whitelist?",
                "Add", new System.Action(() =>
                {
                    GeneralWrappers.ClosePopup();

                    Configuration.GetAntiCrashLimitsConfig().WhitelistedAvatars.Add(avatar.id, true);
                    Configuration.SaveAntiCrashLimitsConfig();

                    PlayerUtils.ReloadAvatarByID(avatar.id);
                }), "Cancel", new System.Action(() =>
                {
                    GeneralWrappers.ClosePopup();
                }));
            ...
            //Avatar does exist - if it is whitelisted then ask to remove
            if (Configuration.GetAntiCrashLimitsConfig().WhitelistedAvatars[avatar.id] == true)
            {
                GeneralWrappers.AlertAction("Whitelist", $"Are you sure you want to remove {avatar.name} from your whitelist?",
                "Remove", new System.Action(() =>
                {
                    GeneralWrappers.ClosePopup();

                    Configuration.GetAntiCrashLimitsConfig().WhitelistedAvatars.Remove(avatar.id);
                    Configuration.SaveAntiCrashLimitsConfig();
            ...
            "Add", new System.Action(() =>
            {
                ...
                Configuration.GetAntiCrashLimitsConfig().WhitelistedAvatars[avatar.id] = true;
                Configuration.SaveAntiCrashLimitsConfig();
